...
| `fm-branch-cleanup.sh`   | Sweep one project's landed firstmate task branches safely across local, origin, and configured no-mistakes refs |
...
| `fm-model-switch.sh`     | Captain-invoked: switch crew-dispatch.json's harness (and optional default effort) and the no-mistakes global config's agent: together |
