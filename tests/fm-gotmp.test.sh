...
make_fake_root() {
  ...
  # fm-branch-merge-lib.sh: teardown shares its landedness proof and branch
  # cleanup lock with the standalone cleanup path.
  ln -s "$ROOT/bin/fm-branch-merge-lib.sh" "$fake/bin/fm-branch-merge-lib.sh"
